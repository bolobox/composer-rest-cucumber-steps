...
class Composer {
     ...
     * error.
     */
    createResource(resource, namespace, name) {
      const fqn = namespace + '.' + name;
      const classDeclaration = this.introspector.getClassDeclaration(fqn);
      return hlclient.createResource(
        this.serializer.toJSON(resource), name, classDeclaration.getIdentifierFieldName(), resource.getIdentifier());
    }
    /**
     * Add a set of assets to an asset registry.
     * @param {Resource} resource The resource to be added
     ...
     * error.
     */
    createTransaction(resource, namespace, name) {
      return hlclient.createTransaction(this.serializer.toJSON(resource), name);
    }
    ...
    addAssets (namespace, name, table) {
        ...
        return resources.reduce((previousPromise, nextResource) => {

          if (previousPromise == undefined) {
            console.log('creating resource on undef ', nextResource.getIdentifier());
            return this.createResource(nextResource, namespace, name);
          }
          else {
            previousPromise.then(() => {
                console.log('creating resource on promise resolved ', nextResource.getIdentifier());
                return this.createResource(nextResource, namespace, name);
            })
          }
        }, Promise.resolve());
    ...
    addParticipants (namespace, name, table) {
        ...
        resources.reduce((previousPromise, nextResource) => {

          if (previousPromise == undefined) {
            console.log('creating participant on undef ', nextResource.getIdentifier());
            return this.createResource(nextResource, namespace, name);
          }
          else {
            previousPromise.then(() => {
                console.log('creating participant on promise resolved ', nextResource.getIdentifier());
                return this.createResource(nextResource, namespace, name);
            })
          }
        }, Promise.resolve() );
    ...
    submitTransactions (namespace, name, table) {
        ...
        resources.reduce((previousPromise, nextResource) => {
          ...
          if (previousPromise == undefined) {
            console.log('creating transaction on undef ', nextResource.getIdentifier());
            return this.createTransaction(nextResource, namespace, name);
          }
          else {
            previousPromise.then(() => {
                console.log('creating transaction on promise resolved ', nextResource.getIdentifier());
                return this.createTransaction(nextResource, namespace, name);
            })
          }
        }, Promise.resolve());
